...
// 4. Bind variables to the PDO state. If any
...
try {
    ...
    $sql = "SELECT product_name, product_image, product_inStock, product_price, product_description, product_status, product_update_date From wdv341_products";
    ...
    $stmt->setFetchMode(PDO::FETCH_ASSOC); 
        ...
        $message = "There has been a problem. The system admininstrator has been contacted. Please try again later!";

        error_log($e->getMessage());        //Delivers a developer defined error message to the PHP log file
        error_log($e->getLine());
        error_log(var_dump(debug_backtrace()));

        //clean up any variable or connections that have been left hanging by this error
...
<head>
    ...
    <style>
        *,:after,:before{-moz-box-sizing:border-box;-webkit-box-sizing:border-box;box-sizing:border-box}
        ...
        a:focus,a:hover{text-decoration:underline}p{margin:0 0 2rem}
        p span{color:#aaa}header{width:98%;margin:40px auto 0;border-bottom:1px solid #ddd;padding-bottom:40px;text-align:center}
        header p{margin:0}
        ...
        tbody tr td:first-child,tbody tr td:nth-child(3),thead td:first-child,thead td:nth-child(3){width:15%}tbody tr td:nth-child(2),thead td:nth-child(2){width:20%}
        tbody tr td:last-child,thead td:last-child{width:50%}
        @media only screen and (min-width:768px){body{font-size:20px;line-height:30px}h2{font-size:30px;line-height:45px}
        h3{font-size:22px;line-height:45px;margin-top:50px}p{margin-bottom:2rem}h1{font-size:60px}pre{padding:20px;font-size:15px}}
        ...
        .productName{font-size:large;margin:1rem 0 .5rem;text-align:left}.productDesc{margin-left:10px;margin-right:10px;margin:0}
        ...
        .productInventory{margin:.5rem 0;text-align:left}.productLowInventory{color:red}
    </style>
</head>
    ...
    <section>
        ...
        <div class="productBlock">
            <div class="productImage">
                <img src="productImages/monitor.jpg">
            </div>
        ...
        ?>
            <!--stuff goes here - display blocks of data for each row in the database-->
